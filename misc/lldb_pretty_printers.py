...
def create_struct(parent, name, struct_type, inits):
        ...
        if isinstance(field_init, int):
            match struct_data.byte_order:
                ...
                case lldb.eByteOrderBig:
                    byte_order = "big"
            field_bytes = field_init.to_bytes(
                field_size,
                byte_order,
                signed=field.type.GetTypeFlags() & lldb.eTypeIsSigned != 0,
            )
    ...
    struct_data.SetData(
        lldb.SBError(),
        struct_bytes,
        struct_data.byte_order,
        struct_data.GetAddressByteSize(),
    )
...
def zig_IsVariableName(string):
    return (
        string != "_"
        and string not in zig_keywords
        and string not in zig_primitives
        and not zig_integer_type.fullmatch(string)
        and zig_identifier_regex.fullmatch(string)
    )


def zig_IsFieldName(string):
    return string not in zig_keywords and zig_identifier_regex.fullmatch(string)
...
class zig_Slice_SynthProvider:
    ...
    def update(self):
        try:
            ...
            self.len = (
                self.value.GetChildMemberWithName("len").unsigned
                if self.ptr.unsigned > page_size
                else 0
            )
    ...
    def get_child_at_index(self, index):
        ...
        try:
            return self.ptr.CreateChildAtOffset(
                "[%d]" % index, index * self.elem_size, self.elem_type
            )
        except:
            return None
...
def zig_String_decode(value, offset=0, length=None):
    try:
        ...
        data = value.GetChildMemberWithName("ptr").GetPointeeData(
            offset,
            length
            if length is not None
            else value.GetChildMemberWithName("len").unsigned,
        )
        b = bytes(data.uint8)
        b = b.replace(b"\\", b"\\\\")
        b = b.replace(b"\n", b"\\n")
        ...
        return (
            s
            if s.isprintable()
            else "".join((c if c.isprintable() else "\\x%02x" % ord(c) for c in s))
        )
    except:
        return None
...
def zig_String_SummaryProvider(value, _=None):
    return '"%s"' % zig_String_decode(value)
...
class zig_Optional_SynthProvider:
    ...
    def update(self):
        try:
            self.child = self.value.GetChildMemberWithName(
                "some"
            ).unsigned == 1 and self.value.GetChildMemberWithName("data").Clone("child")
        except:
            pass
    ...
    def get_child_index(self, name):
        return 0 if self.child and (name == "child" or name == "?") else -1

    def get_child_at_index(self, index):
        return self.child if self.child and index == 0 else None
...
class zig_ErrorUnion_SynthProvider:
    ...
    def update(self):
        try:
            ...
            self.payload = (
                self.value.GetChildMemberWithName("value").Clone("payload")
                if self.error_set.unsigned == 0
                else None
            )
        except:
            pass
    ...
    def get_child_index(self, name):
        return 0 if name == ("payload" if self.payload else "error_set") else -1

    def get_child_at_index(self, index):
        return self.payload or self.error_set if index == 0 else None
...
class zig_TaggedUnion_SynthProvider:
    ...
    def update(self):
        try:
            ...
            self.payload = self.value.GetChildMemberWithName(
                "payload"
            ).GetChildMemberWithName(self.tag.value)
        except:
            pass
    ...
    def get_child_at_index(self, index):
        return (self.tag, self.payload)[index] if index in range(2) else None
...
class std_SegmentedList_SynthProvider:
    ...
    def update(self):
        try:
            self.prealloc_segment = self.value.GetChildMemberWithName(
                "prealloc_segment"
            )
            self.dynamic_segments = zig_Slice_SynthProvider(
                self.value.GetChildMemberWithName("dynamic_segments")
            )
    ...
    def get_child_at_index(self, index):
        try:
            ...
            if index < prealloc_item_count:
                return self.prealloc_segment.child[index]
            prealloc_exp = prealloc_item_count.bit_length() - 1
            shelf_index = (
                log2_int(index + 1)
                if prealloc_item_count == 0
                else log2_int(index + prealloc_item_count) - prealloc_exp - 1
            )
            shelf = self.dynamic_segments.get_child_at_index(shelf_index)
            box_index = (
                (index + 1) - (1 << shelf_index)
                if prealloc_item_count == 0
                else index
                + prealloc_item_count
                - (1 << ((prealloc_exp + 1) + shelf_index))
            )
            elem_type = shelf.type.GetPointeeType()
            return shelf.CreateChildAtOffset(
                "[%d]" % index, box_index * elem_type.size, elem_type
            )
        except:
            return None
...
class std_MultiArrayList_SynthProvider:
    ...
    def update(self):
        try:
            ...
            for helper in self.value.target.FindFunctions(
                "%s.dbHelper" % value_type.name, lldb.eFunctionNameTypeFull
            ):
                ptr_self_type, ptr_child_type, ptr_field_type, ptr_entry_type = (
                    helper.function.type.GetFunctionArgumentTypes()
                )
                if ptr_self_type.GetPointeeType() == value_type:
                    break
            else:
                return
    ...
    def get_child_at_index(self, index):
        try:
            ...
            for field in self.entry_type.fields:
                ...
                data.Append(
                    self.bytes.CreateChildAtOffset(
                        field.name, offset + index * field_size, field_type
                    ).address_of.data
                )
        ...
        try:
            ...
            self.fields = {
                ...
                for index, member in enumerate(
                    ptr_field_type.GetPointeeType().enum_members
                )
            }
            self.entry_type = ptr_entry_type.GetPointeeType()
    ...
    def get_child_at_index(self, index):
        try:
            ...
            for field in self.entry_type.fields:
                field_type = field.type.GetPointeeType()
                data.Append(
                    self.ptrs.child[self.fields[field.name.removesuffix("_ptr")]]
                    .CreateChildAtOffset(
                        field.name, index * field_type.size, field_type
                    )
                    .address_of.data
                )
...
def MultiArrayList_Entry(type):
    return (
        "^multi_array_list\\.MultiArrayList\\(%s\\)\\.Entry__struct_[1-9][0-9]*$" % type
    )
...
class std_HashMapUnmanaged_SynthProvider:
    ...
    def update(self):
        try:
            ...
            for helper in self.value.target.FindFunctions(
                ...
                ptr_self_type, ptr_hdr_type, ptr_entry_type = (
                    helper.function.type.GetFunctionArgumentTypes()
                )
                if ptr_self_type.GetPointeeType() == value_type:
                    break
            ...
            hdr = self.metadata.CreateValueFromAddress(
                "header", self.metadata.deref.load_addr - hdr_type.size, hdr_type
            )
            ...
            self.indices = tuple(
                ...
                for i, value in enumerate(
                    self.metadata.GetPointeeData(0, self.capacity).sint8
                )
                if value < 0
            )
        except:
    ...
    def get_child_at_index(self, index):
        try:
            fields = {
                name: base.CreateChildAtOffset(
                    name, self.indices[index] * pointee_type.size, pointee_type
                ).address_of.data
                for name, base, pointee_type in (
                    (name, base, base.type.GetPointeeType())
                    for name, base in (
                        ("key_ptr", self.keys),
                        ("value_ptr", self.values),
                    )
                )
            }
            data = lldb.SBData()
            for field in self.entry_type.fields:
                data.Append(fields[field.name])
            return self.metadata.CreateValueFromData(
                "[%d]" % index, data, self.entry_type
            )
        except:
            return None
...
class std_Entry_SynthProvider:
    ...
    def update(self):
        try:
            self.children = tuple(
                child.Clone(child.name.removesuffix("_ptr"))
                for child in self.value.children
                if child.type.GetPointeeType().size != 0
            )
    ...
    def get_child_at_index(self, index):
        return (
            self.children[index].deref if index in range(len(self.children)) else None
        )
...
class TagAndPayload_SynthProvider:
    ...
    def update(self):
        try:
            self.tag = self.value.GetChildMemberWithName(
                "tag"
            ) or self.value.GetChildMemberWithName("tag_ptr").deref.Clone("tag")
            data = self.value.GetChildMemberWithName(
                "data_ptr"
            ) or self.value.GetChildMemberWithName("data")
            self.payload = data.GetChildMemberWithName(
                "payload"
            ).GetChildMemberWithName(data.GetChildMemberWithName("tag").value)
        except:
            pass
...
def InstRef_SummaryProvider(value, _=None):
    return (
        value
        if any(value.unsigned == member.unsigned for member in value.type.enum_members)
        else (
            "InternPool.Index(%d)" % value.unsigned
            if value.unsigned < 0x80000000
            else "instructions[%d]" % (value.unsigned - 0x80000000)
        )
    )
...
def InstIndex_SummaryProvider(value, _=None):
    return (
        "instructions[%d]" % value.unsigned
        if value.unsigned < 0x80000000
        else "temps[%d]" % (value.unsigned - 0x80000000)
    )
...
class zig_DeclIndex_SynthProvider:
    ...
    def update(self):
        try:
            ...
            if not ip:
                return
            self.ptr = (
                ip.GetChildMemberWithName("allocated_decls")
                .GetChildAtIndex(self.value.unsigned)
                .address_of.Clone("decl")
            )
        except:
            pass
    ...
    def get_child_index(self, name):
        return 0 if name == "decl" else -1

    def get_child_at_index(self, index):
        return self.ptr if index == 0 else None
...
class Module_Namespace__Module_Namespace_Index_SynthProvider:
    ...
    def update(self):
        try:
            ...
            if not ip:
                return
            self.ptr = (
                ip.GetChildMemberWithName("allocated_namespaces")
                .GetChildAtIndex(self.value.unsigned)
                .address_of.Clone("namespace")
            )
        except:
            pass
    ...
    def get_child_index(self, name):
        return 0 if name == "namespace" else -1
    ...
    def update(self):
        try:
            ...
            for helper in self.value.target.FindFunctions(
                ...
                ptr_self_type, ptr_tag_to_payload_map_type = (
                    helper.function.type.GetFunctionArgumentTypes()
                )
            ...
            tag_to_payload_map = {
                ...
                for field in ptr_tag_to_payload_map_type.GetPointeeType().fields
            }
            ...
            if tag.unsigned < page_size:
                ...
                self.payload = (
                    ptr_otherwise.Cast(tag_to_payload_map[self.tag.value])
...
def Module_Decl_name(decl):
    error = lldb.SBError()
    return decl.process.ReadCStringFromMemory(
        decl.GetChildMemberWithName("name").deref.load_addr, 256, error
    )


def Module_Namespace_RenderFullyQualifiedName(namespace):
    parent = namespace.GetChildMemberWithName("parent")
    if parent.unsigned < page_size:
        return (
            zig_String_decode(
                namespace.GetChildMemberWithName("file_scope").GetChildMemberWithName(
                    "sub_file_path"
                )
            )
            .removesuffix(".zig")
    ...
    return ".".join(
        (
            Module_Namespace_RenderFullyQualifiedName(parent),
            Module_Decl_name(
                namespace.GetChildMemberWithName("ty")
                .GetChildMemberWithName("payload")
                .GetChildMemberWithName("owner_decl")
...
def Module_Decl_RenderFullyQualifiedName(decl):
    return ".".join(
        (
            Module_Namespace_RenderFullyQualifiedName(
                decl.GetChildMemberWithName("src_namespace")
            ),
...
def OwnerDecl_RenderFullyQualifiedName(payload):
    return Module_Decl_RenderFullyQualifiedName(
        payload.GetChildMemberWithName("owner_decl").GetChildMemberWithName("decl")
    )
...
def InternPool_Find(thread):
    for frame in thread:
        ...
        if ip:
            return ip
        mod = (
            frame.FindVariable("zcu")
            or frame.FindVariable("mod")
            or frame.FindVariable("module")
        )
        if mod:
...
class InternPool_Index_SynthProvider:
    ...
    def update(self):
        try:
            ...
            for helper in self.value.target.FindFunctions(
                "%s.dbHelper" % index_type.name, lldb.eFunctionNameTypeFull
            ):
                ptr_self_type, ptr_tag_to_encoding_map_type = (
                    helper.function.type.GetFunctionArgumentTypes()
                )
                if ptr_self_type.GetPointeeType() == index_type:
                    break
            else:
                return
            tag_to_encoding_map = {
                field.name: field.type
                for field in ptr_tag_to_encoding_map_type.GetPointeeType().fields
            }

            ip = InternPool_Find(self.value.thread)
            if not ip:
                return
            self.item = ip.GetChildMemberWithName("items").GetChildAtIndex(
                self.value.unsigned
            )
            ...
            for encoding_field in encoding_type.fields:
                if encoding_field.name == "data":
                    if encoding_field.type.IsPointerType():
                        ...
                        self.data = (
                            extra.GetChildAtIndex(extra_index)
                            .address_of.Cast(encoding_field.type)
                            .deref.Clone("data")
                        )
                    ...
                    for trailing_field in encoding_field.type.fields:
                        trailing_data.Append(
                            extra.GetChildAtIndex(extra_index).address_of.data
                        )
                        trailing_len = dynamic_values[
                            "trailing.%s.len" % trailing_field.name
                        ].unsigned
                        trailing_data.Append(
                            lldb.SBData.CreateDataFromInt(
                                trailing_len, trailing_data.GetAddressByteSize()
                            )
                        )
                        extra_index += trailing_len
                    self.trailing = self.data.CreateValueFromData(
                        "trailing", trailing_data, encoding_field.type
                    )
                else:
                    for path in (
                        encoding_field.type.GetPointeeType()
                        .name.removeprefix("%s::" % encoding_type.name)
                        .removeprefix("%s." % encoding_type.name)
                        .partition("__")[0]
                        .split(" orelse ")
                    ):
    ...
    def num_children(self):
        return 2 + (self.trailing is not None)

    def get_child_index(self, name):
        try:
            return ("tag", "data", "trailing").index(name)
        except:
            return -1

    def get_child_at_index(self, index):
        return (
            (self.tag, self.data, self.trailing)[index] if index in range(3) else None
        )
...
def InternPool_NullTerminatedString_SummaryProvider(value, _=None):
    try:
        ...
        if not ip:
            return
        items = ip.GetChildMemberWithName("string_bytes").GetChildMemberWithName(
            "items"
        )
        b = bytearray()
        i = 0
...
def type_Type_pointer(payload):
    ...
    sentinel = payload.GetChildMemberWithName("sentinel").GetChildMemberWithName(
        "child"
    )
    ...
    if size == "One":
        ...
        summary = "[*c"
    if sentinel:
        summary += ":%s" % value_Value_SummaryProvider(sentinel)
    if size != "One":
        summary += "]"
    if allowzero:
        summary += "allowzero "
    if align != 0 or host_size != 0 or vector_index.value != "none":
        summary += "align(%d%s%s) " % (
            align,
            ":%d:%d" % (bit_offset, host_size)
            if bit_offset != 0 or host_size != 0
            else "",
            ":?"
            if vector_index.value == "runtime"
            else ":%d" % vector_index.unsigned
            if vector_index.value != "none"
            else "",
        )
    if addrspace != "generic":
        summary += "addrspace(.%s) " % addrspace
    if const:
        summary += "const "
    if volatile:
        summary += "volatile "
...
def type_Type_function(payload):
    ...
    comptime_params = (
        payload.GetChildMemberWithName("comptime_params")
        .GetPointeeData(0, len(param_types))
        .uint8
    )
    ...
    return "fn(%s)%s%s %s" % (
        ", ".join(
            tuple(
                "".join(
                    (
                        "comptime " if comptime_param else "",
                        "noalias " if noalias_bits & 1 << i else "",
                        type_Type_SummaryProvider(param_type),
                    )
                )
                for i, (comptime_param, param_type) in enumerate(
                    zip(comptime_params, param_types)
                )
            )
            + (("...",) if is_var_args else ())
        ),
        " align(%d)" % alignment if alignment != 0 else "",
        " callconv(.%s)" % cc if cc != "Unspecified" else "",
...
def type_Type_SummaryProvider(value, _=None):
    ...
    return type_tag_handlers.get(tag, lambda payload: tag)(
        value.GetChildMemberWithName("payload")
    )
...
type_tag_handlers = {
    ...
    "error_union": lambda payload: "%s!%s"
    % (
        type_Type_SummaryProvider(payload.GetChildMemberWithName("error_set")),
        type_Type_SummaryProvider(payload.GetChildMemberWithName("payload")),
    ),
    ...
    % (
        payload.GetChildMemberWithName("len").unsigned,
        value_Value_SummaryProvider(payload.GetChildMemberWithName("sentinel")),
        type_Type_SummaryProvider(payload.GetChildMemberWithName("elem_type")),
    ),
    ...
    "single_const_pointer": lambda payload: "*const %s"
    % type_Type_SummaryProvider(payload),
    "single_mut_pointer": lambda payload: "*%s" % type_Type_SummaryProvider(payload),
    "many_const_pointer": lambda payload: "[*]const %s"
    % type_Type_SummaryProvider(payload),
    "many_mut_pointer": lambda payload: "[*]%s" % type_Type_SummaryProvider(payload),
    "c_const_pointer": lambda payload: "[*c]const %s"
    % type_Type_SummaryProvider(payload),
    ...
    "optional_single_mut_pointer": lambda payload: "?*%s"
    % type_Type_SummaryProvider(payload),
    "optional_single_const_pointer": lambda payload: "?*const %s"
    % type_Type_SummaryProvider(payload),
    ...
    % ",".join(
        zig_String_AsIdentifier(child.GetChildMemberWithName("key"), zig_IsFieldName)
        for child in payload.GetChildMemberWithName("entries").children
    ),
    "error_set_inferred": lambda payload: '@typeInfo(@typeInfo(@TypeOf(%s)).@"fn".return_type.?).error_union.error_set'
    % OwnerDecl_RenderFullyQualifiedName(payload.GetChildMemberWithName("func")),
...
def value_Value_str_lit(payload):
    for frame in payload.thread:
        ...
        return
    return '"%s"' % zig_String_decode(
        mod.GetChildMemberWithName("string_literal_bytes").GetChildMemberWithName(
            "items"
        ),
...
def value_Value_SummaryProvider(value, _=None):
    tag = value.GetChildMemberWithName("tag").value
    return value_tag_handlers.get(tag, lambda payload: tag.removesuffix("_type"))(
        value.GetChildMemberWithName("payload")
    )
...
value_tag_handlers = {
    ...
    "int_big_negative": lambda payload: "-%s"
    % value_tag_handlers["int_big_positive"](payload),
    ...
    "comptime_field_ptr": lambda payload: "&%s"
    % value_Value_SummaryProvider(payload.GetChildMemberWithName("field_val")),
    "elem_ptr": lambda payload: "(%s)[%d]"
    % (
        value_Value_SummaryProvider(payload.GetChildMemberWithName("array_ptr")),
        payload.GetChildMemberWithName("index").unsigned,
    ),
    "field_ptr": lambda payload: "(%s).field[%d]"
    % (
        value_Value_SummaryProvider(payload.GetChildMemberWithName("container_ptr")),
    ...
    "enum_literal": lambda payload: ".%s"
    % zig_String_AsIdentifier(payload, zig_IsFieldName),
    "enum_field_index": lambda payload: "field[%d]" % payload.unsigned,
    "error": lambda payload: "error.%s"
    % zig_String_AsIdentifier(payload.GetChildMemberWithName("name"), zig_IsFieldName),
    "eu_payload": value_Value_SummaryProvider,
    "eu_payload_ptr": lambda payload: "&((%s).* catch unreachable)"
    % value_Value_SummaryProvider(payload.GetChildMemberWithName("container_ptr")),
    "opt_payload": value_Value_SummaryProvider,
    "opt_payload_ptr": lambda payload: "&(%s).*.?"
    % value_Value_SummaryProvider(payload.GetChildMemberWithName("container_ptr")),
...
class root_InternPool_Local_List_SynthProvider:
    ...
    def update(self):
        capacity = self.value.EvaluateExpression(
            "@as(*@This().Header, @alignCast(@ptrCast(@this().bytes - @This().bytes_offset))).capacity"
        )
        self.view = create_struct(
            self.value,
            ".view",
            self.value.type.FindDirectNestedType("View"),
            {
                "bytes": self.value.GetChildMemberWithName("bytes"),
                "len": capacity,
                "capacity": capacity,
            },
...
def root_InternPool_Index_SummaryProvider(value, _=None):
    ...
    tag_value = tag.value
    summary = (
        tag.CreateValueFromType(tag.type)
        .GetChildMemberWithName("encodings")
        .GetChildMemberWithName(
            tag_value.removeprefix(".")
            .removeprefix('@"')
            .removesuffix('"')
            .replace(r"\"", '"')
        )
        .GetChildMemberWithName("summary")
    )
    if not summary:
        return tag_value
    return re.sub(
        expr_path_re,
        lambda matchobj: getattr(
            unwrapped.GetValueForExpressionPath(matchobj[1]), matchobj[2]
        ).strip(matchobj[3] or ""),
...
class root_InternPool_Index_SynthProvider:
    ...
    def update(self):
        ...
        ip = (
            self.value.CreateValueFromType(unwrapped_type)
            .GetChildMemberWithName("debug_state")
            .GetChildMemberWithName("intern_pool")
            .GetNonSyntheticValue()
            .GetChildMemberWithName("?")
        )
        tid_shift_30 = ip.GetChildMemberWithName("tid_shift_30").unsigned
        self.unwrapped = create_struct(
            self.value,
            ".unwrapped",
            unwrapped_type,
            {
                "tid": wrapped >> tid_shift_30,
                "index": wrapped & (1 << tid_shift_30) - 1,
            },
        )
    ...
    def get_child_index(self, name):
        try:
            return ("unwrapped",).index(name)
        except:
            pass

    def get_child_at_index(self, index):
        try:
            return (self.unwrapped,)[index]
        except:
            pass
...
class root_InternPool_Index_Unwrapped_SynthProvider:
    ...
    def update(self):
        self.tag, self.index, self.data, self.payload, self.trailing = (
            None,
            None,
            None,
            None,
        ...
        ip = (
            self.value.CreateValueFromType(self.value.type)
            .GetChildMemberWithName("debug_state")
            .GetChildMemberWithName("intern_pool")
            .GetNonSyntheticValue()
            .GetChildMemberWithName("?")
        ...
        shared = (
            ip.GetChildMemberWithName("locals")
            .GetSyntheticValue()
            .child[self.value.GetChildMemberWithName("tid").unsigned]
            .GetChildMemberWithName("shared")
        )
        item = (
            shared.GetChildMemberWithName("items")
            .GetChildMemberWithName("view")
            .child[index.unsigned]
        )
        self.tag, item_data = (
            item.GetChildMemberWithName("tag"),
            item.GetChildMemberWithName("data"),
        )
        encoding = (
            self.tag.CreateValueFromType(self.tag.type)
            .GetChildMemberWithName("encodings")
            .GetChildMemberWithName(
                self.tag.value.removeprefix(".")
                .removeprefix('@"')
                .removesuffix('"')
                .replace(r"\"", '"')
            )
        ...
        (
            ...
            encoding.GetChildMemberWithName("index"),
            encoding.GetChildMemberWithName("data"),
            encoding.GetChildMemberWithName("payload"),
            encoding.GetChildMemberWithName("trailing"),
            encoding.GetChildMemberWithName("config"),
        )
        if encoding_index:
            ...
            index_data.SetData(
                lldb.SBError(),
                index_bytes,
                index_data.byte_order,
                index_data.GetAddressByteSize(),
            )
            self.index = self.value.CreateValueFromData(
                ".index", index_data, index_type
            )
        elif encoding_data:
            ...
            data_data.SetData(
                lldb.SBError(),
                data_bytes,
                data_data.byte_order,
                data_data.GetAddressByteSize(),
            )
            self.data = self.value.CreateValueFromData(".data", data_data, data_type)
        elif encoding_payload:
            extra = (
                shared.GetChildMemberWithName("extra")
                .GetChildMemberWithName("view")
                .GetChildMemberWithName("0")
            )
            ...
            for payload_field in payload_type.fields:
                ...
                extra_index += 1
            self.payload = create_struct(
                self.value, ".payload", payload_type, payload_fields
            )
            if encoding_trailing and encoding_config:
                trailing_type = encoding_trailing.GetValueAsType()
                trailing_bytes, trailing_data = (
                    bytearray(trailing_type.size),
                    lldb.SBData(),
                )

                def eval_config(config_name):
                    expr = (
                        encoding_config.GetChildMemberWithName(config_name)
                        .summary.removeprefix(".")
                        .removeprefix('@"')
                        .removesuffix('"')
                        .replace(r"\"", '"')
                    )
                    if "payload." in expr:
                        return self.payload.EvaluateExpression(
                            expr.replace("payload.", "@this().")
                        )
                        ...
                        for expr_part in expr_parts[1:]:
                            field = next(
                                filter(
                                    lambda field: field.name == expr_part,
                                    field_type.fields,
                                )
                            )
                        ...
                        field_bytes = trailing_bytes[
                            field_byte_offset : field_byte_offset + field_type.size
                        ]
                        field_data.SetData(
                            lldb.SBError(),
                            field_bytes,
                            field_data.byte_order,
                            field_data.GetAddressByteSize(),
                        )
                        return self.value.CreateValueFromData(
                            ".%s" % expr_parts[-1], field_data, field_type
                        )
                    else:
                ...
                for trailing_field in trailing_type.fields:
                    ...
                    while True:
                        match [
                            trailing_field_type_field.name
                            for trailing_field_type_field in trailing_field_type.fields
                        ]:
                            case ["has_value", "?"]:
                                has_value_field, child_field = (
                                    trailing_field_type.fields
                                )
                                trailing_field_name = "%s.%s" % (
                                    trailing_field_name,
                                    child_field.name,
                                )
                                match eval_config(trailing_field_name).value:
                                    case "true":
                                        if has_value_field.type.name == "bool":
                                            trailing_bytes[
                                                trailing_field_byte_offset
                                                + has_value_field.byte_offset
                                            ] = True
                                        trailing_field_type = child_field.type
                                        trailing_field_byte_offset += (
                                            child_field.byte_offset
                                        )
                                    case "false":
                                        break
                            case ["ptr", "len"]:
                                ptr_field, len_field = trailing_field_type.fields
                                ptr_field_byte_offset, len_field_byte_offset = (
                                    trailing_field_byte_offset + ptr_field.byte_offset,
                                    trailing_field_byte_offset + len_field.byte_offset,
                                )
                                trailing_bytes[
                                    ptr_field_byte_offset : ptr_field_byte_offset
                                    + ptr_field.type.size
                                ] = extra.child[extra_index].address_of.data.uint8
                                len_field_value = eval_config(
                                    "%s.len" % trailing_field_name
                                )
                                len_field_size = len_field.type.size
                                match trailing_data.byte_order:
                                    case lldb.eByteOrderLittle:
                                        len_field_bytes = len_field_value.data.uint8[
                                            :len_field_size
                                        ]
                                        trailing_bytes[
                                            len_field_byte_offset : len_field_byte_offset
                                            + len(len_field_bytes)
                                        ] = len_field_bytes
                                    case lldb.eByteOrderBig:
                                        len_field_bytes = len_field_value.data.uint8[
                                        ...
                                        len_field_end = (
                                            len_field_byte_offset + len_field_size
                                        )
                                        trailing_bytes[
                                            len_field_end
                                            - len(len_field_bytes) : len_field_end
                                        ] = len_field_bytes
                                extra_index += (
                                    ptr_field.type.GetPointeeType().size
                                    * len_field_value.unsigned
                                    + 3
                                ) // 4
                                break
                            case _:
                                for offset in range(0, trailing_field_type.size, 4):
                                    trailing_bytes[
                                        trailing_field_byte_offset
                                        + offset : trailing_field_byte_offset
                                        + offset
                                        + 4
                                    ] = extra.child[extra_index].data.uint8
                                    extra_index += 1
                                break
                trailing_data.SetData(
                    lldb.SBError(),
                    trailing_bytes,
                    trailing_data.byte_order,
                    trailing_data.GetAddressByteSize(),
                )
                self.trailing = self.value.CreateValueFromData(
                    ".trailing", trailing_data, trailing_type
                )
    ...
    def num_children(self):
        return (
            1
            + ((self.index or self.data or self.payload) is not None)
            + (self.trailing is not None)
        )
    ...
    def get_child_at_index(self, index):
        try:
            return (self.tag, self.index or self.data or self.payload, self.trailing)[
                index
            ]
        except:
            pass
...
def root_InternPool_String_SummaryProvider(value, _=None):
    ...
    if wrapped == (1 << 32) - 1:
        return ""
    ip = (
        value.CreateValueFromType(value.type)
        .GetChildMemberWithName("debug_state")
        .GetChildMemberWithName("intern_pool")
        .GetNonSyntheticValue()
        .GetChildMemberWithName("?")
    ...
    string = (
        local_value.GetChildMemberWithName("shared")
        .GetChildMemberWithName("strings")
        .GetChildMemberWithName("view")
        .GetChildMemberWithName("0")
        .child[wrapped & (1 << tid_shift_32) - 1]
        .address_of
    )
...
class root_InternPool_TrackedInst_Index_SynthProvider:
    ...
    def update(self):
        ...
        self.tracked_inst = (
            local_value.GetChildMemberWithName("shared")
            .GetChildMemberWithName("tracked_insts")
            .GetChildMemberWithName("view")
            .GetChildMemberWithName("0")
            .child[wrapped & (1 << tid_shift_32) - 1]
        )
    ...
    def has_children(self):
        return (
            False
            if self.tracked_inst is None
            else self.tracked_inst.GetNumChildren(1) > 0
        )

    def num_children(self):
        return 0 if self.tracked_inst is None else self.tracked_inst.GetNumChildren()

    def get_child_index(self, name):
        return (
            -1
            if self.tracked_inst is None
            else self.tracked_inst.GetIndexOfChildWithName(name)
        )
    ...
    def get_child_at_index(self, index):
        return (
            None
            if self.tracked_inst is None
            else self.tracked_inst.GetChildAtIndex(index)
        )
...
class root_InternPool_Nav_Index_SynthProvider:
    ...
    def update(self):
        ...
        self.nav = (
            local_value.GetChildMemberWithName("shared")
            .GetChildMemberWithName("navs")
            .GetChildMemberWithName("view")
            .child[wrapped & (1 << tid_shift_32) - 1]
        )

    def has_children(self):
        return False if self.nav is None else self.nav.GetNumChildren(1) > 0

    def num_children(self):
        return 0 if self.nav is None else self.nav.GetNumChildren()

    def get_child_index(self, name):
        return -1 if self.nav is None else self.nav.GetIndexOfChildWithName(name)

    def get_child_at_index(self, index):
        return None if self.nav is None else self.nav.GetChildAtIndex(index)
...
def add(
    ...
    *,
    ...
    prefix = ".".join(
        (__name__, (identifier or type).replace(".", "_").replace(":", "_"))
    )
    if summary:
        debugger.HandleCommand(
            'type summary add --category %s%s%s "%s"'
            % (
                category,
                " --inline-children"
                if inline_children
                    ...
                    (
                        ...
                        " --python-function %s_SummaryProvider" % prefix
                        if summary == True
                        else ' --summary-string "%s"' % summary,
                    )
                ),
                " --regex" if regex else "",
                type,
    ...
    if synth:
        debugger.HandleCommand(
            'type synthetic add --category %s%s --python-class %s_SynthProvider "%s"'
            % (category, " --regex" if regex else "", prefix, type)
        )
...
def __lldb_init_module(debugger, _=None):
    ...
    add(
        ...
        category="zig.std",
        regex=True,
        type="^segmented_list\\.SegmentedList\\(.*\\)$",
        identifier="std_SegmentedList",
        synth=True,
        expand=True,
        summary="len=${var.len}",
    )
    add(
        ...
        category="zig.std",
        regex=True,
        type="^multi_array_list\\.MultiArrayList\\(.*\\)$",
        identifier="std_MultiArrayList",
        synth=True,
        expand=True,
    ...
    add(
        ...
        category="zig.std",
        regex=True,
        type="^multi_array_list\\.MultiArrayList\\(.*\\)\\.Slice$",
        identifier="std_MultiArrayList_Slice",
        synth=True,
        expand=True,
        summary="len=${var.len} capacity=${var.capacity}",
    )
    add(
        ...
        category="zig.std",
        regex=True,
        type=MultiArrayList_Entry(".*"),
        identifier="std_Entry",
        synth=True,
        inline_children=True,
        summary=True,
    )
    add(
        ...
        category="zig.std",
        regex=True,
        type="^hash_map\\.HashMapUnmanaged\\(.*\\)$",
        identifier="std_HashMapUnmanaged",
        synth=True,
        expand=True,
        summary=True,
    )
    add(
        ...
        category="zig.std",
        regex=True,
        type="^hash_map\\.HashMapUnmanaged\\(.*\\)\\.Entry$",
        identifier="std_Entry",
        synth=True,
        inline_children=True,
        summary=True,
    ...
    add(
        ...
        category="zig.stage2",
        regex=True,
        type=MultiArrayList_Entry("Zir\\.Inst"),
        identifier="TagAndPayload",
        synth=True,
        inline_children=True,
        summary=True,
    ...
    add(
        ...
        category="zig.stage2",
        regex=True,
        type="^Zir\\.Inst\\.Data\\.Data__struct_[1-9][0-9]*$",
        inline_children=True,
        summary=True,
    )
    add(
    ...
    add(
        debugger,
        category="zig.stage2",
        type="Zir.Inst::Zir.Inst.Index",
        identifier="InstIndex",
        summary=True,
    )
    add(
        debugger,
    ...
    add(
        ...
        category="zig.stage2",
        regex=True,
        type=MultiArrayList_Entry("Air\\.Inst"),
        identifier="TagAndPayload",
        synth=True,
        inline_children=True,
        summary=True,
    ...
    add(
        ...
        category="zig.stage2",
        regex=True,
        type="^Air\\.Inst\\.Data\\.Data__struct_[1-9][0-9]*$",
        inline_children=True,
        summary=True,
    )
    ...
    add(
        debugger,
        category="zig.stage2",
        type="Module.Namespace::Module.Namespace.Index",
        synth=True,
    )
    add(
        debugger,
        category="zig.stage2",
        type="Module.LazySrcLoc",
        identifier="zig_TaggedUnion",
        synth=True,
    )
    ...
    add(
        debugger,
        category="zig.stage2",
        type="InternPool.NullTerminatedString",
        summary=True,
    )
    add(
        debugger,
    ...
    add(
        debugger,
        category="zig.stage2",
        type="InternPool.Key.Int.Storage",
        identifier="zig_TaggedUnion",
        synth=True,
    )
    add(
        debugger,
        category="zig.stage2",
        type="InternPool.Key.ErrorUnion.Value",
        identifier="zig_TaggedUnion",
        synth=True,
    )
    add(
        debugger,
        category="zig.stage2",
        type="InternPool.Key.Float.Storage",
        identifier="zig_TaggedUnion",
        synth=True,
    )
    add(
    ...
    add(
        debugger,
        category="zig.stage2",
        type="InternPool.Key.Aggregate.Storage",
        identifier="zig_TaggedUnion",
        synth=True,
    )
    add(
        debugger,
        category="zig.stage2",
        type="arch.x86_64.CodeGen.MCValue",
        identifier="zig_TaggedUnion",
        synth=True,
        inline_children=True,
        summary=True,
    ...
    add(
        ...
        category="zig",
        regex=True,
        type=r"^root\.InternPool\.Local\.List\(.*\)$",
        identifier="root_InternPool_Local_List",
        synth=True,
        expand=True,
        summary="capacity=${var%#}",
    )
    add(
        debugger, category="zig", type="root.InternPool.Index", synth=True, summary=True
    )
    ...
    add(
        ...
        category="zig",
        regex=True,
        type=r"^root\.InternPool\.(Optional)?(NullTerminated)?String$",
        identifier="root_InternPool_String",
        summary=True,
    )
    add(
        debugger,
        category="zig",
        regex=True,
        type=r"^root\.InternPool\.TrackedInst\.Index(\.Optional)?$",
        identifier="root_InternPool_TrackedInst_Index",
        synth=True,
    )
    add(
        debugger,
        category="zig",
        regex=True,
        type=r"^root\.InternPool\.Nav\.Index(\.Optional)?$",
        identifier="root_InternPool_Nav_Index",
        synth=True,
    )
